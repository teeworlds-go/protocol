...
// from a given byte slice. The ownership of that byte slice it transfered to the unpacker
// and MUST not be accessed afterwards. If you want to continue working with the remaining bytes,
// then call the .Bytes() method in order to retrieve the remaining bytes that were not consumed.
// The unpacker consumes bytes when one of the NextXXX() methods are called.
...
func UnpackInt(data []byte) (i int, err error) {
	i, n := varint.Varint(data)
	if n == 0 {
...
func (u *Unpacker) NextBool() (bool, error) {
	...
	case 0x00:
		b = false
	case 0x01: // TODO: do we want to explicitly require 0x01 here or is it supposed to be != 0x00
		b = true
	default:
